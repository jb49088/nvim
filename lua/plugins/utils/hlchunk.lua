    ...
    config = function()
        ...
        Snacks.toggle({
            ...
            set = function(enabled)
                ...
                if enabled then
                    ...
                    -- Try triggering cursor event to refresh chunks
        ...
        vim.api.nvim_create_autocmd({ "WinEnter" }, {
            ...
            callback = function()
                ...
                -- Only enable hlchunk if the toggle is currently on
                if hlchunk_enabled then
                    vim.cmd("EnableHLChunk")
                end
        ...
        local current_win = vim.api.nvim_get_current_win()
        if not is_floating_win(current_win) and hlchunk_enabled then
            vim.cmd("EnableHLChunk")
            last_regular_win = current_win
        end
    end,
